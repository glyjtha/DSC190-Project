...
# Convert rna_counts to a matrix (if necessary for Seurat)
rna_counts_matrix <- as.matrix(rna_counts[, -1, with = FALSE])  # Drop first column if it's gene names
rownames(rna_counts_matrix) <- rna_counts[[1]]  # Assign first column as rownames
```
...
```{r}
seurat_obj <- CreateSeuratObject(counts = rna_counts_matrix, meta.data = metadata)
```
```{r}
